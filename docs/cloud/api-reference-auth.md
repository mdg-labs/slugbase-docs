...
All paths are under `{BASE_URL}/api/auth`. In Cloud, authentication is session-based; tenant is determined by the current organization in session. **Cloud:** `GET /api/auth/setup/status` always returns `{ "initialized": true }`. `POST /api/auth/register` is the main signup flow (email verification required).
